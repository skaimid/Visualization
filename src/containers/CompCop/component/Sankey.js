import {
  ...
  const Rect = ({ index, x0, x1, y0, y1, name, value, length, colors,selectedCluster }) => {
  ...
  const Sankey = ({data, version, setSelectedCluster, selectedCluster}) => {
    ...
    if (data) {
      ...
      return (
        <svg viewBox="0 0 512 512"  preserveAspectRatio="xMidYMid meet" className="w-full h-full">
          <g>
            {links.map((d, i) => (
              <Link
                data={d}
      ...
      return <div className=" text-center  text-3xl underline rounded-lg h-[512px]">
